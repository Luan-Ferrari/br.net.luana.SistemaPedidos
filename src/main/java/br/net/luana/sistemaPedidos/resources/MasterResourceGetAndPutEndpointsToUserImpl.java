...
@RestController
public abstract class MasterResourceGetAndPutEndpointsToUserImpl<T extends MasterDomain, D extends MasterDTOImpl, ID extends Integer>
        implements MasterResourceGetAndPutEndpointsToUser<T, D, ID> {

    @Autowired
    private D dto;
    private MasterService masterService;

    public MasterResourceGetAndPutEndpointsToUserImpl(MasterService masterService) {
    ...
    public ResponseEntity<Void> update(D dto, ID entityId) {
        T entity = (T) masterService.updateById((T)dto.makeEntityFromDTO(dto), entityId);
